...
```plaintext
# 모든 서비스는 LoadBalancer를 통해 접근
172.16.10.3    haproxy.local
172.16.10.3    gitlab.local
172.16.10.3    jenkins.local
172.16.10.3    harbor.local
172.16.10.3    grafana.local
172.16.10.3    prometheus.local
172.16.10.3    sonarqube.local
172.16.10.3    security.local
172.16.10.3    next-demo.local
172.16.10.3    nest-demo.local
172.16.10.3    python-demo.local
```
...
## 통신 흐름 설명
1. 외부 접근 (호스트 PC -> 서비스)
   - 호스트 PC에서 도메인 접속 시 모든 요청이 HAProxy로 라우팅
   - HAProxy가 적절한 백엔드 서버로 요청 전달
   - SSL 종단 및 인증서 처리는 HAProxy에서 수행
...
2. hosts 파일에 의해 모든 요청이 LoadBalancer(172.16.10.3)로 전달
3. HAProxy가 요청의 도메인을 확인하여 적절한 백엔드 서버로 라우팅:
...
| 서비스 | URL | 설명 |
|--------|-----|------|
| HAProxy Stats | https://haproxy.local:8404 | HAProxy 상태 모니터링 |
